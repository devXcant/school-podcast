...
export default async function handler(
  ...
  try {
    ...
    if (!title || !courseId || !file) {
      return res
        .status(400)
        .json({ message: "Title, course, and file are required" });
    }
    ...
    const isAssignedCourseRep =
      isCourseRep && course.course_rep === session.user.id;

    if (!isAdmin && !isCourseOwner && !isAssignedCourseRep) {
      return res
        .status(403)
        .json({ message: "Permission denied for this course" });
    }

    const fileContent = fs.readFileSync(file.filepath);
    const fileExtension = path.extname(file.originalFilename || "");
    const fileName = `${uuidv4()}${fileExtension}`;
    const filePath = `podcasts/${
